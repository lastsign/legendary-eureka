...
def cipher(text, k, alpha, cip_text):
    ciphered = []
    ind_val = {i: alpha[i] for i in range(len(alpha))}
    val_ind = {alpha[i]: i for i in range(len(alpha))}
    for i, x in enumerate(text):
        if x != ' ':
            ciphered.append(ind_val[(val_ind[x] + val_ind[k[i % len(k)]]) % len(alpha)])
        else:
            ciphered.append(x)
...
def check_fields(text, k, v0, v1):
    ...
    elif v0 == 'ru' and not bool(set(k).issubset(set(alpha_rus + ' '))):
        messagebox.showerror('Проверка ключа', 'Некоректные символы для русского алфавита')
    elif v1 == 'eng' and not bool(set(k).issubset(set(alpha_eng + ' '))):
        messagebox.showerror('Проверка ключа', 'Некоректные символы для английского алфавита')
    else:
        return True
    return False
...
def decipher(text, k, alpha, cip_text):
    ciphered = []
    ind_val = {i: alpha[i] for i in range(len(alpha))}
    val_ind = {alpha[i]: i for i in range(len(alpha))}
    for i, x in enumerate(text):
        if x != ' ':
            ciphered.append(ind_val[(val_ind[x] + len(alpha) - val_ind[k[i % len(k)]]) % len(alpha)])
        else:
            ciphered.append(x)
    cip_text.config(state='normal')
    cip_text.delete(1.0, END)
...
def decipher_text():
    ...
    elif check_fields(cipher_text.get(1.0, 'end-1c'), k, v0, v1):
        cipher_label['text'] = ''
        decipher_label['text'] = 'Расшифрованный текст'
        if v1 == 'eng':
            decipher(cipher_text.get(1.0, 'end-1c'), k, alpha_eng, decipher_text)
            cipher_text.delete(1.0, END)
        elif v0 == 'ru':
            decipher(cipher_text.get(1.0, 'end-1c'), k, alpha_rus, decipher_text)
            cipher_text.delete(1.0, END)
